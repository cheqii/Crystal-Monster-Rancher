...
{     //splitting an Item
    ...
    public void OnPointerDown(PointerEventData data)                    //splitting the item now
    {
        ...
        if (transform.parent.parent.parent.GetComponent<Hotbar>() == null && data.button == PointerEventData.InputButton.Left && pressingButtonToSplit && inv.stackable && (inv.ItemsInInventory.Count < (inv.height * inv.width))) //if you press leftclick and and keycode
        {
            ...
            if (itemOnObject.itemInventory.itemValue > 1)                                                                         //we split the item only when we have more than 1 in the stack
            {
                int splitPart = itemOnObject.itemInventory.itemValue;                                                           //we take the value and store it in there
                itemOnObject.itemInventory.itemValue = (int)itemOnObject.itemInventory.itemValue / 2;                                     //calculate the new value for the splitted item
                splitPart = splitPart - itemOnObject.itemInventory.itemValue;                                                   //take the different

                inv.addItemToInventory(itemOnObject.itemInventory.itemID, splitPart);                                            //and add a new item to the inventory
                inv.stackableSettings();

                if (GetComponent<ConsumeItem>().duplication != null)
                {
                    GameObject dup = GetComponent<ConsumeItem>().duplication;
                    dup.GetComponent<ItemOnObject>().itemInventory.itemValue = itemOnObject.itemInventory.itemValue;
